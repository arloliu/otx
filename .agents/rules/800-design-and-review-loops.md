...
For architectural changes, create or update the design or implementation plan
under `docs/design/` (or `docs/plans/`) and get approval before coding.
...
Grounded in otx, as reminders — verify before relying on them:

- **Invariant framing:** "traces, logs, and metrics always resolve the same
  resource identity" — not "the log exporter used a different service name once."
- **Enumerate paths:** before changing endpoint defaults, list every resolver
  that reads the endpoint (config validation, `exporter.go`, `zaplog/core.go`,
  each protocol's exporter) — they must agree on the resolved value.
- **Grep, don't claim:** "`internal/endpoint` is the only place that parses
  endpoint strings" is a grep claim — run it before relying on it.
