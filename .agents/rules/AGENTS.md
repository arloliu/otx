...
- Add `700` only for per-request / per-log hot paths (span helpers, middleware,
  `zaplog` core) or code handling external config, endpoints, or credentials.
...
- Hot paths / external config / auth: `700-performance-security.md`.
